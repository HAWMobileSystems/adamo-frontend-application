                ...
                db.query('SELECT sess FROM session')
                    .then(function (sessions) {
                        ...
                        if (users.indexOf(user.username) >= 0) {
                            ...
                            throw ({message: 'user already logged in', success: false});
                        }
...
app.get('/logout', function (req, res) {
    try {
        if (req.session.user) {
            req.session.destroy();
            res.status(200).send({message: 'logging out', success: true});
        }
...
app.get('/user', function (req, res) {
    try {
        if (req.session.user) {
            res.status(200).send({message: 'logging in', success: true, data: req.session.user});
        }
        else throw({status: 400, data: {message: 'not logged in', success: false}});
    }
    catch (error) {
        ...
        if (req.session.user) {
            db.query('SELECT sess FROM session')
                .then(function (sessions) {
                    ...
                    res.status(200).send({message: 'message', success: true, data: users});
                })
                .catch(function (error) {
